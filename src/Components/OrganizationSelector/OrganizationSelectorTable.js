...
function EnhancedTable(props) {
    const { rowSelected, setRowSelected, orgType, onDblClickRow } = props;
    ...
    useEffect(() => {
        ...
        if (reload) {
            (async () => {
                ...
                setLoading(true)
                try {
                    const result = await organizationAPI().getAll(pagination, searchedKeyword, {
    ...
    const searchKeyword = async (keyWord) => {
        ...
        try {
            const result = await organizationAPI().search(keyWord, pagination)
            if (result.status === 200) {
                if (result.data.data) {
                    setRows(result.data.data.organization)
    ...
    const handleRowClick = (event, item) => {
        switch (event.detail) {
            case 1: {
                const selectedIndex = rowSelected.findIndex((row) => row.organizationId === item.organizationId);
                let newSelected = [];
    ...
    const isRowSelected = (id) => (rowSelected.findIndex((row) => row.organizationId === id)) !== -1;
...
EnhancedTable.propTypes = {
    ...
    onDblClickRow: PropTypes.func.isRequired
};

export const OrganizationSelectorTable = React.memo(EnhancedTable);
