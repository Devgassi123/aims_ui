...
const headCells = [
    ...
    { id: 'poStatus', numeric: false, disablePadding: false, label: 'Order Status' },
];
        ...
        >
            ...
            {numSelected > 0 ? (
                ...
                <Box display="flex">
                    ...
                    <Tooltip title="New P.O.">
                        <IconButton aria-label="add P.O." onClick={onAddPO}>
                            <AddIcon />
                        </IconButton>
                    </Tooltip>
                </Box>
...
function EnhancedTable(props) {
    ...
    const [orderBy, setOrderBy] = React.useState('poId');
    ...
    useEffect(() => {
        ...
        if (reload) {
            (async () => {
                ...
                try {
                    const result = await purchaseOrderAPI().aggregated(pagination, searchedKeyword, filters)
                    if (result.status === 200) {
                        if (result.data.data) {
                            if (!isMounted) return;
                            setRows(result.data.data.po);
                                ...
                                : rows.length === 0
                                    ...
                                    : stableSort(rows, getComparator(order, orderBy))
                                        .map((row, index) => {
                                            ...
                                            const isRowItemSelected = isRowSelected(row.poId);
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell component="th" id={labelId} scope="row" padding="none" onClick={(event) => handleRowClick(event, row.poId)}>
                                                        {pagination.rowFrom + index}
                                                    </TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.poId)}>{moment(row.orderDate).format("MM-DD-YYYY HH:mm")}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.poId)}>{row.poId}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.poId)}>{row.refNumber}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.poId)}>{row.supplierName}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.poId)}
                                                        style={{
                                                            color: clsx({
                                                                "#2196f3": row.poStatus.includes("Created"),
                                                                "green": row.poStatus.includes("Fully"),
                                                                "orange": row.poStatus.includes("Partial"),
                                                                "red": row.poStatus.includes("Cancel"),
                                                            })
                                                        }}
                                                    >
                                                        {row.poStatus}
...
export const POTable = React.memo(EnhancedTable);
