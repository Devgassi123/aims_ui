...
// API
import { purchaseOrderDetailsAPI } from '../../../../../../redux/api/api';
// FUNCTIONS
...
const headCells = [
    { id: 'poLineId', numeric: false, disablePadding: true, label: 'Order Line', width: 'small' },
    { id: 'poLineStatus', numeric: false, disablePadding: false, label: 'Status', width: 'medium' },
    ...
    // { id: 'poId', numeric: false, disablePadding: false, label: 'PO #' },
    { id: 'orderQty', numeric: false, disablePadding: false, label: 'Order Qty', width: 'small' },
...
function EnhancedTable(props) {
    const { poId, rowSelected, setRowSelected, reload, setReload, setHideItemDetails, setReloadHdrTable, setSoftReloadHdrTable } = props;
    ...
    useEffect(() => {
        ...
        if (reload || (poId !== null)) {
            (async () => {
                ...
                try {
                    const result = await purchaseOrderDetailsAPI().getByPOId(poId, pagination)
            ...
            swal({
                ...
                if (willDelete) {
                    new Promise((resolve, reject) => {
                        selected.forEach(async (poLineId, index, array) => {
                            try {
                                const result = await purchaseOrderDetailsAPI().delete(poLineId)
                                if (result.status === 200) {
                                    if (result.data.code === 0) {
                                        reject(`System could not delete ${poLineId}: ${result.data.message}`);
                                        return;
                                    }
                                }
                            } catch (error) {
                                reject(`Error occurred in deleting ${poLineId}!`);
                                return;
                            }
    ...
    const onReceiveItem = () => {
        if(selected.length > 1) {
            ...
            const toReceiveItem = rows.filter((value) => value.poLineId === selected[0])
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = rows.map((n) => n.poLineId);
    ...
    const handleRowClick = (event, selectedRow) => {
        // const selectedIndex = rowSelected.findIndex((item) => item.poLineId === selectedRow.poLineId);
        // let newSelected = [];
    ...
    const isRowSelected = (id) => rowSelected.findIndex((item) => item.poLineId === id) !== -1;
                                    ...
                                    ? (
                                        <TableRow>
                                            <TableCell colSpan={headCells.length + 1} align="center">
                                                <Typography variant="h5" className={classes.typography}>No order items yet.</Typography>
                                            </TableCell>
                                        </TableRow>
                                    )
                                    : stableSort(rows, getComparator(order, orderBy))
                                        .map((row, index) => {
                                            const isItemSelected = isSelected(row.poLineId);
                                            const isRowItemSelected = isRowSelected(row.poLineId);
                                            ...
                                            return (
                                                <TableRow
                                                    ...
                                                    tabIndex={-1}
                                                    key={row.poLineId}
                                                    // onClick={(event) => handleRowClick(event, row)} //still trigerred when user ticked the checkbox
                                                ...
                                                >
                                                    <TableCell padding="checkbox" >
                                                        <Checkbox
                                                            ...
                                                            onClick={(event) => handleClick(event, row.poLineId)}
                                                        />
                                                    </TableCell>
                                                    ...
                                                    {/* <TableCell align="left">{row.poId}</TableCell> */}
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row)}>{Number(row.orderQty).toLocaleString()}</TableCell>
...
export const POItemsTable = React.memo(EnhancedTable);
