...
// API
import { purchaseOrderAPI, receiveTransfersAPI } from '../../../../redux/api/api';
// FUNCTIONS
...
import ReceiveTransfersFilters from './ReceiveTransfersFilters';
import Pagination from '../../../Pagination/Pagination';
import ImportExport from '../ImportExport/ImportExport';

const headCells = [
    { id: 'row', numeric: false, disablePadding: true, label: 'Row' },
    { id: 'transferDate', numeric: false, disablePadding: false, label: 'Transfer Date' },
    { id: 'whTransferId', numeric: false, disablePadding: false, label: 'Transfer #' },
    { id: 'refNumber', numeric: false, disablePadding: false, label: 'Reference #' },
    { id: 'whFrom', numeric: false, disablePadding: false, label: 'Warehouse From' },
    { id: 'whTransStatusId', numeric: false, disablePadding: false, label: 'Transfer Status' },
];
        ...
        >
            ...
            {numSelected > 0 ? (
                ...
                <Box display="flex">
                    ...
                    <Tooltip title="New WH Transfer">
                        <IconButton aria-label="add WH Transfer" onClick={onAddPO}>
                            <AddIcon />
                        </IconButton>
                    </Tooltip>
                </Box>
...
function EnhancedTable(props) {
    ...
    useEffect(() => {
        ...
        if (reload) {
            (async () => {
                ...
                try {
                    const result = await receiveTransfersAPI().aggregated(pagination, searchedKeyword, filters)
                    if (result.status === 200) {
                        if (result.data.data) {
                            if (!isMounted) return;
                            setRows(result.data.data.whTransfer);
                    ...
                    addToast("Error occurred in getting warehouse transfers!", {
                        appearance: "error"
                    })
                    if (!isMounted) return
                    setLoading(false);
    ...
    const onCancelPO = () => {
        swal({
            title: "Are you sure?",
            text: "Once cancelled, you will not be able to recover this transfer document!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            ...
            if (willCancel) {
                try {
                    const result = await receiveTransfersAPI().delete(selected[0])
                    if (result.status === 200) {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = rows.map((n) => n.whTransferId);
                            ...
                            {loading
                                ...
                                : rows.length === 0
                                    ...
                                    : stableSort(rows, getComparator(order, orderBy))
                                        .map((row, index) => {
                                            const isItemSelected = isSelected(row.whTransferId);
                                            const isRowItemSelected = isRowSelected(row.whTransferId);
                                                ...
                                                >
                                                    <TableCell padding="checkbox">
                                                        <Checkbox
                                                            ...
                                                            onClick={(event) => handleClick(event, row.whTransferId)}
                                                        />
                                                    </TableCell>
                                                    <TableCell component="th" id={labelId} scope="row" padding="none" onClick={(event) => handleRowClick(event, row.whTransferId)}>
                                                        {pagination.rowFrom + index}
                                                    </TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.whTransferId)}>{moment(row.transferDate).format("MM-DD-YYYY HH:mm")}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.whTransferId)}>{row.whTransferId}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.whTransferId)}>{row.refNumber}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.whTransferId)}>{row.whFrom}</TableCell>
                                                    <TableCell align="left" onClick={(event) => handleRowClick(event, row.whTransferId)}
                                                        style={{
                                                            color: clsx({
                                                                "#2196f3": row.whTransferStatus.includes("Created"),
                                                                "green": row.whTransferStatus.includes("Fully"),
                                                                "orange": row.whTransferStatus.includes("Partial"),
                                                                "red": row.whTransferStatus.includes("Cancel"),
                                                            })
                                                        }}
                                                    >
                                                        {row.whTransferStatus}
                                                    </TableCell>
                                                </TableRow>
                                            );
                                        })}
...
export const ReceiveTransfersTable = React.memo(EnhancedTable);
