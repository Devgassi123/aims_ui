...
// API
import { receiveTransfersDetailsAPI } from '../../../../../../../redux/api/api';
// FUNCTIONS
...
const headCells = [
    { id: 'row', numeric: false, disablePadding: true, label: 'Line #' },
    { id: 'whTransLineStatus', numeric: false, disablePadding: false, label: 'Status' },
...
function EnhancedTable(props) {
    const { whTransferId, rowSelected, setRowSelected, reload, setReload, setHideItemDetails, setReloadHdrTable, setSoftReloadHdrTable } = props;
    ...
    useEffect(() => {
        ...
        if (whTransferId !== null) {
            ...
            (async () => {
                ...
                try {
                    const result = await receiveTransfersDetailsAPI().getByTransferId(whTransferId, pagination)
                    if (result.status === 200) {
                        if (result.data.code === 0) {
                            ...
                            if (result.data.data) {
                                if (!isMounted) return;
                                setRows(result.data.data.whTransDetailModel);
            ...
            swal({
                ...
                if (willDelete) {
                    new Promise((resolve, reject) => {
                        selected.forEach(async (whTransferLineId, index, array) => {
                            try {
                                const result = await receiveTransfersDetailsAPI().delete(whTransferLineId)
                                if (result.status === 200) {
                                    if (result.data.code === 0) {
                                        reject(`System could not delete ${whTransferLineId}: ${result.data.message}`);
                                        return;
                                    }
                                }
                            } catch (error) {
                                reject(`Error occurred in deleting ${whTransferLineId}!`);
                                return;
                            }
    ...
    const onReceiveItem = () => {
        if (selected.length > 1) {
            ...
            const toReceiveItem = rows.filter((value) => value.whTransferLineId === selected[0])
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = rows.map((n) => n.whTransferLineId);
    ...
    const isRowSelected = (id) => rowSelected.findIndex((item) => item.whTransferLineId === id) !== -1;
            ...
            <Paper className={classes.paper} elevation={0}>
                ...
                >
                    <Table
                        ...
                        size='small'
                        aria-label="warehouse transfer items table"
                        stickyHeader
                    >
                        <EnhancedTableHead
                            classes={classes}
                            ...
                            {loading
                                ...
                                : rows.length === 0
                                    ...
                                    : stableSort(rows, getComparator(order, orderBy))
                                        .map((row, index) => {
                                            const isItemSelected = isSelected(row.whTransferLineId);
                                            const isRowItemSelected = isRowSelected(row.whTransferLineId);
                                                ...
                                                >
                                                    <TableCell padding="checkbox">
                                                        <Checkbox
                                                            ...
                                                            onClick={(event) => handleClick(event, row.whTransferLineId)}
                                                        />
                                                    </TableCell>
...
export const TransferredItemsTable = React.memo(EnhancedTable);
