...
const headCells = [
    { id: 'zoneID', numeric: false, disablePadding: true, label: 'Zone ID' },
    { id: 'zoneName', numeric: false, disablePadding: false, label: 'Zone Name' },
    { id: 'warehouseID', numeric: false, disablePadding: false, label: 'Warehouse ID' },
    { id: 'areaName', numeric: false, disablePadding: false, label: 'Area' },
...
const EnhancedTableToolbar = (props) => {
    ...
    const { numSelected, handleOpenFilter } = props;
    ...
    return (
        ...
        >
            ...
            {numSelected > 0 ? (
                ...
                <Box display="flex">
                    <Box mr={1}>
                        <Tooltip title="Search Zone ID, Zone Name, Area Name...">
                            <TextField 
                                id="txtSearch"
                                label="Search"
                                variant="outlined"
...
function EnhancedTable(props) {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = rows.map((n) => n.zoneID);
    ...
    return (
        <div className={classes.root}>
            <ZonesFilters open={openFilter} handleClose={handleCloseFilter} />
            <Paper className={classes.paper}>
                <EnhancedTableToolbar numSelected={selected.length} handleOpenFilter={handleOpenFilter} />
                            ...
                            {loading 
                                ...
                                : rows.length === 0
                                    ...
                                    : stableSort(rows, getComparator(order, orderBy))
                                        ...
                                        .map((row, index) => {
                                            const isItemSelected = isSelected(row.zoneID);
                                            const isRowItemSelected = isRowSelected(row.zoneID);
                                            const labelId = `zone-table-checkbox-${index}`;
                                            ...
                                            return (
                                                <TableRow
                                                    ...
                                                    tabIndex={-1}
                                                    key={row.zoneID}
                                                    onClick={(event) => handleRowClick(event, row.zoneID)}
                                                ...
                                                >
                                                    <TableCell padding="checkbox">
                                                        <Checkbox
                                                            ...
                                                            onClick={(event) => handleClick(event, row.zoneID)}
                                                        />
                                                    </TableCell>
...
export const ZonesTable = React.memo(EnhancedTable);
